...
data <- data %>%
  ...
  filter(cand_party_affiliation == c("REP", "DEM"), entity_type == c("CCM", "CAN", "COM", "IND", "ORG", "PAC", "PTY"), !is.na(c("entity_type", "transaction_type", "transaction_amt", "cand_party_affiliation"))) %>%
  add_count(entity_type)
data
```
```{r}
